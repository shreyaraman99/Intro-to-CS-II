...
vector<string> DecrypterImpl::crack(const string& ciphertext)
{
    ...
    // Get cipher with highest priority
    for (int i = maxLength; i > 0; i--) {
        if (priorityQueue.find(i) != nullptr) {
            ...
            for (auto it = cipherWords.begin(); it != cipherWords.end(); it++) {
                ...
                // CREATE COLLECTION OF CANDIDATES
                vector<string> candidates;
                candidates = m_wordlist.findCandidates(cipherWord, translatedWord);
                
                // IF NO CANDIDATES THROW OUT MAPPING AND RETURN
                ...
                for (auto c = candidates.begin(); c != candidates.end(); c++) {
                    ...
                    // Create mapping based on candidate
                    ...
                    //TRANSLATE ENTIRE MESSAGE USING NEW MAPPING
                    ...
                    // CHECK IF ALL FULLY TRANSLATED WORDS EXIST IN WORD LIST
